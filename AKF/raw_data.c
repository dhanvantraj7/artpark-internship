...
void writeCsvData(const char* filename, DATA data[], int size) {
    ...
    fprintf(file, "AccX,AccY,AccZ,GyroX,GyroY,GyroZ\n");

    for (int i = 0; i < size; ++i) {
        fprintf(file, "%.8f,%.8f,%.8f,%.8f,%.8f,%.8f\n",
...
int main() {
    ...
    // Assuming you want to save the output to a new file, specify the new file's path here
    const char* outputFilePath = "C:\\Users\\Camras\\Documents\\VS code\\AKF\\Processed_Roll_data.csv";
