...
static inline void initializeSensorData(DATA data[], int size, const char* filePath) {
    ...
    readCsvData("C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv", accxValues, 0);
    readCsvData("C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv", accyValues, 1);
    readCsvData("C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv", acczValues, 2);
    readCsvData("C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv", gyrxValues, 3);
    readCsvData("C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv", gyryValues, 4);
    readCsvData("C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv", gyrzValues, 5);
...
void writeQx1CsvData(const char* filename, AKF *Algo, int size) {
    FILE *file = fopen(filename, "w");
    if (!file) {
        printf("Could not open file %s for writing.\n", filename);
        exit(1);
    ...
    fclose(file);
    printf("q_x1 data written to %s successfully.\n", filename);
}
...
int main() {
    ...
    const char* filePath = "C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\Roll_data.csv";
    ...
    const char* q_x1OutputFilePath = "C:\\Users\\Camras\\Documents\\VS code\\AKF\\data\\qx1_data.csv";
...
//     // Initialize your AKF structure, if needed
//     // For example, setting initial values for P_i, Q, R, etc.

//     const char* filePath = "C:\\Users\\Camras\\Documents\\VS code\\AKF\\Roll_data.csv";
//     initializeSensorData(data, NUM_SAMPLES, filePath);

//     // Loop through each data point and call QuaternionPred
//     for (int i = 0; i < NUM_SAMPLES; ++i) {
//         QuaternionPred(&data[i], &Algo);
//         // Store the current q_xT values in the history
//         memcpy(Algo.q_xT_history[i], Algo.q_xT, sizeof(Algo.q_xT));
//     }

//     // Path for the q_xT values CSV file
//     const char* qxTOutputFilePath = "C:\\Users\\Camras\\Documents\\VS code\\AKF\\qxT_data.csv";
//     // Write q_xT data to CSV file after processing all data points
//     writeQxTCsvData(qxTOutputFilePath, &Algo, NUM_SAMPLES);

//     // Path for the output CSV file for roll and pitch values
//     const char* outputFilePath = "C:\\Users\\Camras\\Documents\\VS code\\AKF\\Acc_Roll_data.csv";
//     // Write roll and pitch data to CSV file after processing all data points
//     writeCsvData(outputFilePath, data, NUM_SAMPLES);
...
//     // Write data to CSV file after QuaternionPred is done for all data points
//     writeCsvData(outputFilePath, data, NUM_SAMPLES);
